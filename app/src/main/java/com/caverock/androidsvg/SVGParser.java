...
public class SVGParser extends DefaultHandler2
{
   ...
   private void  parseAttributesGlyph(SVG.Glyph obj, Attributes attributes) throws SAXException
   {
      for (int i=0; i<attributes.getLength(); i++)
      {
         String val = attributes.getValue(i).trim();
         switch (SVGAttr.fromString(attributes.getLocalName(i)))
         {
            case glyph_name:
               ...
               break;
            case unicode:
               obj.glyphUnicode = Html.escapeHtml(val);
   ...
   private void  parseAttributesRect(SvgRect obj, Attributes attributes) throws SAXException
   {
      for (int i=0; i<attributes.getLength(); i++)
      {
         String val = attributes.getValue(i).trim();
   ...
   private void  glyph(Attributes attributes) throws SAXException
   {
      debug("<glyph>");

      if (currentElement == null)
         throw new SAXException("Invalid document. Root element must be <svg>");
      SVG.Glyph  obj = new SVG.Glyph();
      obj.document = svgDocument;
      obj.parent = currentElement;
      parseAttributesCore(obj, attributes);
      parseAttributesStyle(obj, attributes);
      ...
      parseAttributesConditional(obj, attributes);
      parseAttributesPath(obj, attributes);
      parseAttributesGlyph(obj, attributes);
      currentElement.addChild(obj);
   }
